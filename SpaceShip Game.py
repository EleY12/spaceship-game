...
try:
    pygame.mixer.init()
    # LOAD AND LOOP BACKGROUND MUSIC INFINITELY
    pygame.mixer.music.load('Spaceship-Game-BGM.mp3')
    pygame.mixer.music.set_volume(0.8)  # High music volume
...
# SAFE AUDIO SOUND EFFECTS LOADING
        ...
        try:
            with open("highscore.txt", "r") as file:
                for line in file:
                    ...
                    if len(data) == 2:
                        leaderboard.append([int(data[0]), data[1], 1])
                    elif len(data) == 3:
                        leaderboard.append([int(data[0]), data[1], int(data[2])])
        except Exception:
            pass
...
def save_leaderboard():
    ...
    try:
        with open(SCORE_FILE, "w") as file:
            json.dump({"leaderboard": leaderboard[:5]}, file, indent=4)
    except Exception as e:
        print(f"Error saving leaderboard: {e}")

def add_to_leaderboard(new_score, name, level_reached):
    global leaderboard
    leaderboard.append([new_score, name, level_reached])
    leaderboard.sort(key=lambda x: x[0], reverse=True)
    leaderboard = leaderboard[:5]
    save_leaderboard()
...
# PARALLAX STARS SETUP
stars_back = []
stars_mid = []
stars_fore = []
for _ in range(30):
    stars_back.append([random.randint(0, SCREEN_WIDTH), random.randint(0, SCREEN_HEIGHT), random.uniform(0.4, 0.8)])
for _ in range(20):
    stars_mid.append([random.randint(0, SCREEN_WIDTH), random.randint(0, SCREEN_HEIGHT), random.uniform(1.2, 1.8)])
for _ in range(10):
    stars_fore.append([random.randint(0, SCREEN_WIDTH), random.randint(0, SCREEN_HEIGHT), random.uniform(2.5, 3.5)])
...
def select_new_mission():
    global current_mission, mission_desc, mission_progress, mission_start_score, laser_storm_hit_taken, score
    global anomaly_type
...
def spawn_standard_enemies():
    ...
    for _ in range(count):
        enemies.append(get_random_scout(hp=hp_multiplier))
...
while Game_Activated:
    ...
    if game_state == "PLAYING":
        ...
        if player_health <= 0:
            ...
            trigger_shake(50, 25)
            add_to_leaderboard(score, username, level)
            game_state = "GAMEOVER"
    ...
    if game_state == "START":
        title_surf = large_font.render("SPACE SHOOTER", True, (0, 191, 255))
        user_prompt_surf = font.render("Enter Name:", True, (200, 200, 200))
        name_surf = large_font.render(username + "|", True, (255, 255, 255))
        prompt_surf = font.render("Press ENTER to Start", True, (0, 255, 0))
        
        display_surface.blit(title_surf, (50, 50))
        display_surface.blit(user_prompt_surf, (50, 140))
        display_surface.blit(name_surf, (50, 180))
        display_surface.blit(prompt_surf, (50, 480))
        
        lead_title = font.render("🏆 LOCAL LEADERBOARD 🏆", True, (255, 215, 0))
        display_surface.blit(lead_title, (450, 50))
        for idx, entry in enumerate(leaderboard):
            ...
            display_surface.blit(lead_surf, (450, 100 + (idx * 30)))
        ...
        score_surf = font.render(f"Your Score: {score} (Reached Level {level})", True, (255, 255, 255))
        ...
        user_surface = font.render(f"Pilot: {username}", True, (0, 191, 255))
        
        best_val = leaderboard[0][0] if leaderboard else 0
        best_lvl = leaderboard[0][2] if leaderboard and len(leaderboard[0]) > 2 else 1
        best_surface = font.render(f"Best: {best_val} (Lvl {best_lvl})", True, (255, 215, 0))
